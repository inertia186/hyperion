class TagsController < ApplicationController
  ...
  def create_favorite
    tag = current_account.favorite_tags.find_or_create_by(tag: params[:id])
    
    respond_to do |format|
      format.html {
        if tag.persisted?
          redirect_to params[:return_to] || tags_url
        else
          render 'new'
        end
      }
  ...
  def delete_favorite
    count = current_account.favorite_tags.where(tag: params[:id]).destroy_all
